...
x_train, w_train, m_train, y_train, m_pred_train, \
x_test, w_test, m_test, y_test, m_pred_test = createDatasets(tests, config_object)

model = None
if model_type == "ulm":
    model = createUlm(embeddings_file=embeddings_file, 
                      N_train = x_train.shape[1], N_pred = len(tests))
elif model_type == "multi":
...
reduce = ReduceLROnPlateau(monitor="val_loss", factor=0.9, patience=20, verbose=0,  mode="auto",
                           min_delta=0.00001, cooldown=0, min_lr=1e-7)

# Load previous weights if any.

try: model.load_weights(load_file)
except: pass 

if model_type == "ulm":
    model.fit(x=[x_train, w_train, m_train, m_pred_train], y=y_train, shuffle=True, batch_size=32,
             epochs=1000, validation_split=0.2, callbacks=[ tensorboard_callback, early_stopping,
             reduce, tf.keras.callbacks.TerminateOnNaN(), MessagerCallback(), ] )
    ...
    model.fit(x=x_train, y=y_train, shuffle=True, batch_size=32,
             epochs=1000, validation_split=0.2, callbacks=[ tensorboard_callback, early_stopping,
             reduce, tf.keras.callbacks.TerminateOnNaN(), MessagerCallback(), ] )
