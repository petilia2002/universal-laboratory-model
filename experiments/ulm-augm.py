...
sql = (
    " is not null or ".join([("a." + analyte.getViewName()) for analyte in tests])
    + " is not null"
)
...
for data in con.cursor().execute(
    f"""select {sel} from lab2 a 
...
l_out_regression = keras.layers.TimeDistributed(Dense(units=1, activation="sigmoid"))(
    l_o
)
...
def cbcLoss(y_true, y_pred):
    ...
    r_mse = tensorflow.reduce_mean(
        tensorflow.math.square((y_true[1] - y_pred[1]) * mask)
    )
...
x_train, w_train, m_train, y_train, m_pred_train, r_train = prepareData(
    0, train_size, ids, X, W, Y, source, tests, x_map, y_map, MASKED_VALUE
)
x_test, w_test, m_test, y_test, m_pred_test, r_test = prepareData(
    train_size, len(X), ids, X, W, Y, source, tests, x_map, y_map, MASKED_VALUE
)
...
def calcRocs(analyte, key, label, ans):
    ...
    for i in range(len(y_train)):
        ...
        if analyte not in m_pred_train[i]:
            continue
        prop = np.where(m_pred_train[i] == analyte)[0][0]

        if len(set(w_train[i]).intersection(key)) > 0:
            continue
    ...
    for i in range(len(y_test)):
        ...
        if analyte not in m_pred_test[i]:
            continue
        prop = np.where(m_pred_test[i] == analyte)[0][0]

        if len(set(w_test[i]).intersection(key)) > 0:
            continue
...
def calc_stats(analyte, name):
    ...
    for key in keys:
        ...
        fpr, tpr, thresholds, qe, qr, roc_auc, r1, r2 = calcRocs(
            analyte, set(nm), name, ans
        )
        ...
        accuracy = (
            (tp + tn) * 100.0 / (tp + tn + fp + fn) if (tp + tn + fp + fn) != 0 else 0
        )
