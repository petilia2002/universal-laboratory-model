...
class RealValueLayer(tensorflow.keras.Layer):
    ...
    def call(self, inputs):
        ...
        v = tensorflow.reshape(v, shape=(-1, v.shape[1], 1))
        v = tensorflow.repeat(v, repeats=[self.embedding_size], axis=2)
...
class RealValueLayer2(tensorflow.keras.Layer):

    def __init__(self, embedding_size, **kwargs):
        super(RealValueLayer2, self).__init__(**kwargs)
        self.embedding_size = embedding_size
...
class MaskLayerLeft(keras.layers.Layer):
    ...
    def call(self, x):
        ...
        length = K.shape(x)[1]
        rank = tensorflow.ones(shape=(1, length), dtype="float32")
        y = K.expand_dims(x, axis=-1)

        mask = K.dot(y, rank)
        return tensorflow.transpose(mask, (0, 2, 1))


class MaskLayerRight(keras.layers.Layer):
...
l_pred = Input(name="outcomes", shape=(4,))
l_pred = RealValueLayer2(embedding_size=emb_size)(l_pred)
...
l_enc = keras.layers.MultiHeadAttention(num_heads=8, key_dim=16, dropout=0.1)(
    l_v, l_v, l_v, attention_mask=l_m1
)
l_dec = keras.layers.MultiHeadAttention(num_heads=8, key_dim=16, dropout=0.1)(
    l_tests, l_enc, l_enc, attention_mask=l_m2
)

l_o = keras.layers.TimeDistributed(Dense(units=16, activation="relu"))(l_dec)
l_f = Flatten()(l_o)

l_out_cbc = Dense(units=4, activation="sigmoid")(l_f)
...
y_train = np.zeros((train_size, 4), dtype="float32")
...
y_test = np.zeros((test_size, 4), dtype="float32")
...
a = model([x_train[0:2], w_train[0:2], m_train[0:2], m_pred_train[0:2]])
...
calc_stats(0, "ferritin")
calc_stats(1, "glucose")
calc_stats(2, "cholesterin")
calc_stats(3, "uric acid")
