...
def createUlm(embeddings_file, N_train, N_pred):
    ...
    l_v = RealValueLayer(embedding_size = emb_size, 
                         name = "real_value_embedding")([l_emb, l_in])

    # Add trainable bias to the input values. 
    l_pos = Embedding(name='yandex_bias', input_dim = dims[0], 
                      output_dim = emb_size)(l_words);
    l_v = l_v + l_pos;

    # Add trainable bias to the ouytput values. 
    l_p = Embedding(name='yandex_bias_pred', input_dim = dims[0], 
                    output_dim = emb_size)(l_pred)
    l_tests = l_tests + l_p
